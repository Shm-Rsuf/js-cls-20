...
const favColors = ['red', 'green', 'tomato', 'pink', 'crimson', 'black', 'blue', 'purple'];

const favColorsUpperCase = favColors.map((color) => color[0].toUpperCase() + color.slice(1));
...
const maxValue = arr.reduce((acc, vlue) => acc > vlue ? acc : vlue, arr.flat(0))
console.log(maxValue);

const minValue = arr.reduce((acc, value) => acc < value ? acc : value, arr.flat(0))
...
const totalDeposit = movements2.filter((move) => move > 0).map((deposit) => deposit * 1.2).reduce((acc, value) => acc + value, 0)

console.log(totalDeposit);

const moneyRobbed = [15000, 25000, 35000, 14000, 25000, 7500];

const totalMoney = moneyRobbed.filter((money) => money > 10000).map((money) => money / 2).filter((money) => money > 10000).reduce((acc, money) => acc + money, 0)
